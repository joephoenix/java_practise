...
public class ShutterHorization extends Applet implements Runnable {
	...
	public void run() {
		while (true) {
			try {
				...
				while (true) {
					// 水平效果
					for (int i = 0; i < (int) (imageWidth / 10); i++) {
						try {
							// 线程休眠
							Thread.sleep(50);
							for (int j = 0; j < imageWidth; j += (int) (imageWidth / 10)) {
								for (int k = 0; k < imageHeight; k++) {
									pixA[imageWidth * k + (j + i)] = pixB[imageWidth
											* k + (j + i)];
								}
							}
						} catch (InterruptedException e) {
